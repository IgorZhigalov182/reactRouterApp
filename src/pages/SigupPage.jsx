...
const signUpSchema = Yup.object({
  ...
  password: Yup.string().min(4, 'Password must contain at least 4 symbols').required('Required'),
});
  ...
  return (
    <>
      ...
      <FormikProvider value={formik}>
        {!successful && (
          <form className="space-y-6 min-w-[200px] w-full mb-10" onSubmit={formik.handleSubmit}>
            <TextField label="Username" name="username" icon={UserIcon} />
            {/* <FloatingTextInput /> */}
            <TextField label="Email" name="email" icon={MailIcon} />

            <TextField label="Password" name="password" type="password" icon={KeyIcon} />
            <div className="pt-2">
              <Button disabled={loading}>{loading && <SpinLoading />} Sign Up</Button>
            </div>
          </form>
        )}
        {message && (
